...
access starts at the **Expert** plan. Tell the user plainly and point them at
...
customer tools: `gateway_models`, `list_models`, `generate`, `enhance`,
`library`, `list_characters`, `presets`, `templates`,
`recipes`.
...
an id taken from `list_models` (the wide third-party research catalog) instead of
`gateway_models` (what this account can actually run). Re-read `gateway_models` and pick
from it.
...
identifiable person. When this happens on Seedance, `generate` answers with
...
they accept, call `generate` again with the SAME arguments plus `confirmFallback: true`. If they
...
Accepting the fallback keeps the face from the START FRAME and strips every reference — a character
sheet no longer travels. When identity matters more than the frame, decline, move the approved still
into `image_urls` as the last reference, and say "open on reference image N" in the prompt.
